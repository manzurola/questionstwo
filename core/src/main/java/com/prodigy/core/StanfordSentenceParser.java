...
public class StanfordSentenceParser implements POSTagger {
    ...
    @Override
    public List<TaggedWord> tagWords(List<Word> words) {
        ...
        for (int i = 0; i < sourceTagged.size(); i++) {
            ...
            collected.add(new TaggedWord(word.word(), POS.ofValue(word.tag()).orElse(null), i));
        }
